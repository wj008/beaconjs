...
export class Upload extends Beacon.Controller {
    ...
    private createFileName(ext) {
        ...
        str += month;
        return '/' + str + '/' + Beacon.uuid(8) + '.' + ext;
    }

    private  sizeCalculation(img, width, height) {
    ...
    private async upload() {
        let RootPath = path.join(Beacon.RUNTIME_PATH, 'public/upload');
        let file = this.file('upload');
        if (file == null) {
            this.fail('错误的字段名', 2);
        }
        //判断类型
        let mime = Beacon.getConfig('post:allow_upload_types') || {};
    ...
    //验证图片
    private validimg(file, info, RootPath, typedirname, sortname) {
        let size_urls = {};

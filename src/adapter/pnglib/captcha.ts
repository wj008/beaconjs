...
export class Captcha {
    ...
    private depth;
    private dispNumber;
    private widthAverage;

    public constructor(width, height, dispNumber) {
        this.width = width;
        this.height = height;
    ...
    public captchapng() {
        ...
        let numMask = [];
        numMask[0] = this.loadNumMask0();
        numMask[1] = this.loadNumMask1();


        this.depth = 16;

        this.widthAverage = Math.floor(this.width / this.dispNumber.length);
        let that = this;

        var p = new PNGImage(this.width, this.height, this.depth);


        var getRandColor = function (a=100) {
        ...
        var drawImpurities = function (c) {
            let x = Math.floor(Math.random() * that.width);
            let y = Math.floor(Math.random() * that.height);
            let lineIndex=p.index(x,y);
            p.setBuffer(lineIndex, c);
            let tice= Math.floor(Math.random() * 30);
            for(let n=0;n<tice;n++) {
                x += (1 - Math.floor(Math.random() * 2.2));
                y += (1 - Math.floor(Math.random() * 2.2));
                if (x > 0 && x < that.width && y > 0 && y < that.height) {
                    lineIndex = p.index(x, y);
                    p.setBuffer(lineIndex, c);
                }
            }
        }
        let bc=getRandColor(100);
        for(var bx=0;bx<this.width;bx++){
            for(var by=0;by<this.width;by++){
                p.setBuffer(p.index(bx, by),bc);
            }
        }
        let c1=getRandColor();
        for(let n=0;n<10;n++){
        ...
        for (var numSection = 0; numSection < this.dispNumber.length; numSection++) {
            ...
            var font = Math.floor(Math.random() * numMask.length);
            font = (font >= numMask.length ? 0 : font);
            //var random_x_offs = 0, random_y_offs = 0;
            var random_x_offs = Math.floor(Math.random() * (this.widthAverage - numMask[font][dispNum][0].length));
            var random_y_offs = Math.floor(Math.random() * (this.height - numMask[font][dispNum].length));
            random_x_offs = (random_x_offs < 0 ? 0 : random_x_offs);
            random_y_offs = (random_y_offs < 0 ? 0 : random_y_offs);

            let blueColor = p.createColor('blue');
            let r = Math.floor(Math.random() * 255);
            let g = Math.floor(Math.random() * 255);
            let b = Math.floor(Math.random() * 255);
            let randColor = p.color(r, g, b, 255);
            let randColor2 = p.color(255, 255, 255, 180);

            for (var i = 0; (i < numMask[font][dispNum].length) && ((i + random_y_offs) < this.height); i++) {

                let ox = this.widthAverage * numSection + random_x_offs;
                let oy = i + random_y_offs;
                let lineIndex = p.index(ox,oy);
                let lineIndex2 = p.index(ox+1,oy+1);

                for (var j = 0; j < numMask[font][dispNum][i].length; j++) {
                    if ((numMask[font][dispNum][i][j] == '1') && (this.widthAverage * numSection + random_x_offs + j) < this.width) {
                        p.setBuffer(lineIndex + j,randColor );
                        p.setBuffer(lineIndex2+ j,randColor2);
                    }
                }
            }
        }

        let c4=getRandColor(50);
        for(let n=0;n<10;n++){
            drawImpurities(c4);
        }
        let c5=getRandColor(50);
        for(let n=0;n<10;n++){
            drawImpurities(c5);
        }
        let c6=getRandColor(50);
        for(let n=0;n<10;n++){
            drawImpurities(c6);
        }
        return p;
